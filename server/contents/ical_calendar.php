<?php
	 ...
	 *  https://evertpot.com/icalendar-vcard-parser-for-php/
	 *  There you can learn how vobject works.
	 **/
	...
	foreach($calendarAdress as $adress) {
		...
		$today = new DateTime('2010-11-20');
		$today->setTime(00, 00);
		$nextWeek = new DateTime('+7 days');
		$newVCalendar = $calendar->expand( $today, $nextWeek );
	...
	foreach($calData as $event) {
		...
		if( empty($varDate) || $varDate != $datetime->format('d-m-Y') ){
			$cursorX += $fontSize * 1.5;
			$varDate = $datetime->format('d-m-Y');
